...
triads_to_try = [('italy', 'italian', 'korea'), ('india', 'delhi', 'china'), ('man', 'woman', 'engineer'), ('small', 'smaller', 'far')]
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = np.sqrt(abs(1-np.sum(mu_orth**2)))*(e_w1B-mu_B)/np.sqrt(np.sum((e_w1-mu_orth-mu_B)**2))
    corrected_e_w2B = np.sqrt(abs(1-np.sum(mu_orth**2)))*(e_w2B-mu_B)/np.sqrt(np.sum((e_w2-mu_orth-mu_B)**2))
